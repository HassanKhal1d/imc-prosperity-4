...
def save_results_json(
    ...
    output_path:   str = "cwfa_results.json",
) -> str:
    """
    Save a CWFAResult to a JSON file consumable by the dashboard.
    Returns the path written.
    """
    from datetime import datetime

    # Parameter frequency map: "param=val,..." -> count
    param_frequency = {}
    for combo in result.combo_results:
        key = ",".join(f"{k}={v}" for k, v in sorted(combo.best_params.items()))
        param_frequency[key] = param_frequency.get(key, 0) + 1

    # Per-param grid scores: for each param combination, collect OOS scores
    # (only the winning combination per combo is available here)
    ...
    payload = {
        "metadata": {
            ...
            "n_is_combos":    len(grid_combinations(param_grid)),
        },
        "summary": {
            "n_combos_run":       len(result.combo_results),
            "oos_scores":         result.oos_scores.tolist(),
            "oos_mean":           result.oos_mean,
            "oos_std":            result.oos_std,
            "oos_sharpe_proxy":   result.oos_sharpe,
            "recommended_params": result.recommended_params,
            "consensus_rate":     (
                max(result.best_params_counts.values()) / len(result.combo_results)
                if result.combo_results else 0
            ),
        },
        "combo_results": [
            {
                "combo_id":   r.combo_id,
                "oos_blocks": list(r.oos_blocks),
                "is_blocks":  list(r.is_blocks),
