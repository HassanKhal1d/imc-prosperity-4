...
class Trader:
    ...
    def _trade_emeralds(self, order_depth: OrderDepth, position: int, ts: int):
        ...
        # End‑of‑session forced flattening
        if ts >= HARD_CLOSE_TS:
            return self._close_position(
                'EMERALDS', order_depth, position, best_bid, best_ask
            )

        # Tighter skew trigger near end
        skew_trig = LATE_SKEW_POS if ts >= UNWIND_START_TS else EM_SKEW_TRIGGER

        # ── Narrow spread regime (spread = 8): only unwind at FV ─────────────
        if spread == 8:
            # Sub‑state B: best bid = FV → sell to reduce long
            ...
            # Sub‑state A: best ask = FV → buy to reduce short
        ...
        bid_price = EM_FV - EM_INSIDE   # e.g. 9993
        ask_price = EM_FV + EM_INSIDE   # e.g. 10007
    ...
    def _trade_tomatoes(self, order_depth, position, trader_state):
        ...
        # Track EMA history for velocity
